...
<head>
    ...
    <style>
        ...
        body.mode-sand {
            ...
            --border-soft:rgba(107,68,35,0.10); --border-medium:rgba(107,68,35,0.18); --border-strong:rgba(107,68,35,0.30);
            --shadow-soft:0 4px 16px -8px rgba(107,68,35,0.15); --shadow-hover:0 12px 28px -12px rgba(107,68,35,0.25);
        }
        body.mode-linen {
            --bg-page:#ebe0c8; --bg-card:#f5efe0; --bg-card-hover:#faf4e4;
            --bg-section-alt:#e0d4b8; --bg-input:#f5efe0; --bg-input-focus:#fffdf8;
            --text-primary:#2a2622; --text-secondary:#5c5048; --text-muted:#8c7e6a;
            --border-soft:rgba(107,68,35,0.12); --border-medium:rgba(107,68,35,0.22); --border-strong:rgba(107,68,35,0.35);
            --shadow-soft:0 4px 16px -8px rgba(107,68,35,0.12); --shadow-hover:0 12px 28px -12px rgba(107,68,35,0.20);
        }
        body.mode-earth {
            ...
            --border-soft:rgba(212,189,158,0.15); --border-medium:rgba(212,189,158,0.25); --border-strong:rgba(212,189,158,0.40);
            --shadow-soft:0 8px 24px -12px rgba(0,0,0,0.40); --shadow-hover:0 16px 40px -16px rgba(0,0,0,0.60);
        }
        ...
        * { -webkit-font-smoothing: antialiased; }
        ...
        .header-logo {
            ...
            color: #f5efe0; text-decoration: none;
            text-transform: uppercase; white-space: nowrap; flex-shrink: 0;
        }
        .desktop-nav { display: flex; align-items: center; gap: 28px; }
        .desktop-auth { display: flex; align-items: center; gap: 12px; flex-shrink: 0; }
        .burger-btn {
        ...
        .burger-btn span {
            ...
            background: rgba(245,239,224,0.9); border-radius: 2px; transition: all 0.3s;
        }
        ...
        .nav-link::after {
            ...
            height: 1px; background: var(--brown-bright);
            transform: scaleX(0); transform-origin: center; transition: transform 0.3s;
        }
        .nav-link:hover::after, .nav-link.active::after { transform: scaleX(1); }
        .nav-link.active { color: #fff; }
        .header-reg-btn {
            padding: 7px 16px; font-size: 11px; letter-spacing: 2px;
            text-transform: uppercase; border-radius: 10px;
            background: linear-gradient(135deg, var(--brown-bright), var(--brown));
            color: #2a2218; box-shadow: 0 6px 16px -6px rgba(212,165,116,0.5);
            transition: all 0.25s; font-weight: 500; border: 0; cursor: pointer; white-space: nowrap;
        }
        .header-reg-btn:hover { transform: translateY(-2px); }

        .btn {
            display: inline-block; padding: 10px 24px; font-size: 11px;
        ...
        .btn-light:hover { background: #fff; color: var(--bg-page); }
        .btn-filled {
            background: linear-gradient(135deg, var(--brown-bright), var(--brown));
            color: #fff; border-color: var(--brown);
        }
        ...
        .input-field {
            background-color: var(--bg-input); border: 1px solid var(--border-medium);
            color: var(--text-primary); border-radius: 10px; transition: all 0.2s;
            font-family: 'Inter', sans-serif; font-size: 14px; width: 100%;
        }
        .input-field::placeholder { color: var(--text-muted); }
        .input-field:focus { outline: none; border-color: var(--brown); background-color: var(--bg-input-focus); box-shadow: 0 0 0 3px rgba(184,136,88,0.18); }
        ...
        .card:hover { background-color: var(--bg-card-hover); border-color: var(--border-medium); transform: translateY(-3px); box-shadow: var(--shadow-hover); }
        .card-flat { background-color: var(--bg-card); border: 1px solid var(--border-soft); border-radius: 14px; box-shadow: var(--shadow-soft); }

        .divider { display:flex; align-items:center; justify-content:center; gap:16px; font-size:10px; letter-spacing:3px; color:var(--brown-dark); text-transform:uppercase; }
        body.mode-earth .divider { color:var(--brown-bright); }
        .divider::before,.divider::after { content:''; height:1px; flex:1; max-width:80px; background:linear-gradient(90deg,transparent,currentColor,transparent); opacity:0.5; }
        .section-number { font-family:'Karelle',serif; font-size:12px; letter-spacing:3px; color:var(--brown-dark); text-transform:uppercase; }
        body.mode-earth .section-number { color:var(--brown-bright); }
        .tag { display:inline-block; padding:4px 10px; font-size:11px; border-radius:8px; background:var(--bg-input); border:1px solid var(--border-medium); color:var(--brown-dark); transition:all 0.2s; text-decoration:none; white-space:nowrap; }
        body.mode-earth .tag { color:var(--brown-bright); }
        .tag:hover { border-color:var(--brown); transform:translateY(-1px); }
        .badge { display:inline-block; padding:3px 10px; font-size:10px; letter-spacing:2px; text-transform:uppercase; border-radius:8px; font-weight:500; }
        .badge-forest { background:rgba(61,79,51,0.85); color:#fff; }
        .badge-brown { background:linear-gradient(135deg,var(--brown-bright),var(--brown)); color:#fff; }
        .badge-soft { background:var(--bg-input); color:var(--text-secondary); border:1px solid var(--border-soft); }
        .badge-pinned { background:linear-gradient(135deg,var(--forest),var(--forest-dark)); color:#fff; animation:pulse-glow 2.5s ease-in-out infinite; }
        @keyframes pulse-glow { 0%,100%{box-shadow:0 0 0 0 rgba(107,138,92,0.3)} 50%{box-shadow:0 0 0 6px rgba(107,138,92,0)} }
        .gradient-number { font-family:'Karelle',serif; background:linear-gradient(135deg,var(--brown-bright),var(--brown)); -webkit-background-clip:text; background-clip:text; -webkit-text-fill-color:transparent; }
        .avatar { border-radius:50%; overflow:hidden; border:2px solid var(--border-medium); background:var(--bg-input); transition:all 0.3s; display:inline-block; flex-shrink:0; }
        .avatar:hover { border-color:var(--brown); transform:scale(1.05); }
        .avatar-square { border-radius:10px; overflow:hidden; border:1px solid var(--border-medium); background:var(--bg-input); box-shadow:var(--shadow-soft); flex-shrink:0; }
        .avatar-fallback { display:flex; align-items:center; justify-content:center; width:100%; height:100%; background:linear-gradient(135deg,var(--forest),#6b8a5c); color:#fff; font-family:'Karelle',serif; }
        .vote-btn { background:transparent; border:0; font-size:18px; color:var(--text-secondary); cursor:pointer; transition:all 0.2s; padding:4px 8px; border-radius:6px; display:block; }
        ...
        .modal-overlay { position:fixed; inset:0; background:rgba(0,0,0,0.7); backdrop-filter:blur(8px); z-index:50; display:none; align-items:flex-start; justify-content:center; padding:5vh 16px; overflow-y:auto; }
        .modal-overlay.active { display:flex; animation:fadeIn 0.25s; }
        @keyframes fadeIn { from{opacity:0} to{opacity:1} }
        .modal-card { background:var(--bg-card); border:1px solid var(--border-medium); border-radius:18px; padding:32px 28px; width:100%; max-width:440px; box-shadow:0 24px 60px -16px rgba(0,0,0,0.6); position:relative; animation:slideUp 0.4s cubic-bezier(0.2,0.8,0.2,1); }
        @keyframes slideUp { from{transform:translateY(20px);opacity:0} to{transform:translateY(0);opacity:1} }
        .modal-close { position:absolute; top:12px; right:14px; width:32px; height:32px; border:0; border-radius:50%; background:transparent; color:var(--text-secondary); font-size:22px; cursor:pointer; transition:all 0.2s; }
        .modal-close:hover { background:var(--bg-input); color:var(--text-primary); transform:rotate(90deg); }

        .reveal { opacity:0; transform:translateY(30px); transition:opacity 0.7s cubic-bezier(0.2,0.8,0.2,1),transform 0.7s cubic-bezier(0.2,0.8,0.2,1); }
        .reveal.visible { opacity:1; transform:translateY(0); }
        .reveal-delay-1{transition-delay:0.1s} .reveal-delay-2{transition-delay:0.2s} .reveal-delay-3{transition-delay:0.3s}

        .flash-message { border-radius:12px; border-left:3px solid var(--success); background:rgba(107,138,92,0.12); color:var(--text-primary); animation:slideDown 0.4s cubic-bezier(0.2,0.8,0.2,1); padding:12px 20px; font-size:14px; margin-bottom:16px; }
        .flash-error { border-left-color:var(--error); background:rgba(196,86,83,0.12); }
        @keyframes slideDown { from{opacity:0;transform:translateY(-10px)} to{opacity:1;transform:translateY(0)} }

        .footer-bg { background:linear-gradient(135deg,var(--forest-dark),#1f2a1c); border-top:1px solid rgba(0,0,0,0.2); color:#f5efe0; }

        ::-webkit-scrollbar{width:8px} ::-webkit-scrollbar-track{background:var(--bg-page)} ::-webkit-scrollbar-thumb{background:var(--border-medium);border-radius:4px} ::-webkit-scrollbar-thumb:hover{background:var(--brown)}
        html{scroll-behavior:smooth}

        .forum-layout { display:grid; grid-template-columns:1fr 280px; gap:24px; }
        .forum-sidebar { display:block; }

        .news-layout { display:grid; grid-template-columns:1fr 280px; gap:24px; }
        ...
        .service-layout { display:grid; grid-template-columns:1fr 300px; gap:32px; align-items:start; }
        .service-aside { position:sticky; top:80px; }

        .service-photos-grid { display:grid; grid-template-columns:repeat(auto-fill, minmax(220px,1fr)); gap:12px; margin-bottom:28px; }
        .service-photo-item { width:100%; aspect-ratio:4/3; overflow:hidden; border-radius:12px; box-shadow:var(--shadow-soft); }
        .service-photo-item img { width:100%; height:100%; object-fit:cover; transition:transform 0.4s; display:block; }
        ...
        @media (max-width: 1023px) {
            ...
            .forum-layout { grid-template-columns: 1fr 230px; gap: 16px; }
            .news-layout { grid-template-columns: 1fr 220px; gap: 16px; }
            .profile-layout { grid-template-columns: 240px 1fr; gap: 16px; }
            .service-layout { grid-template-columns: 1fr 260px; gap: 20px; }
        }
        ...
        @media (max-width: 767px) {
            ...
            .service-aside { position: static !important; top: auto !important; }

            .service-photos-grid { grid-template-columns: 1fr 1fr; gap: 8px; }
            ...
            .profile-tabs { overflow-x: auto; flex-wrap: nowrap !important; padding-bottom: 4px; }
            .tab-btn { white-space: nowrap; flex-shrink: 0; padding: 8px 14px !important; font-size: 9px !important; }
            ...
            .section-py { padding-top: 32px !important; padding-bottom: 32px !important; }

            .form-grid-2 { grid-template-columns: 1fr !important; }

            .forum-sort-bar { flex-wrap: wrap !important; gap: 8px !important; }
            .forum-found { margin-left: 0 !important; width: 100%; margin-top: 4px; }
        }
        ...
        @media (max-width: 480px) {
            ...
            .hero-content { padding: 36px 14px !important; }

            .grid-2-to-1 { grid-template-columns: 1fr !important; }
        }
        ...
        <div class="header-inner">
            ...
            <div class="desktop-auth">
                @auth
                    ...
                    <a href="{{ route('notifications.index') }}" class="nav-link" title="Уведомления"
            ...
            <button class="burger-btn" id="burger" type="button" aria-label="Меню">
...
<footer class="footer-bg" style="padding:40px 0; margin-top:64px;">
    <div class="workspace" style="text-align:center;">
        ...
        <div class="divider" style="margin-bottom:12px;color:var(--brown-bright);"><span>est. 2026</span></div>
